...
## Chim_gen.py - Generates random chimeras using an input FASTA file.

## USAGE: python chim_gen.py infile.fa # > output.fa
...
def main(argv):
    ...
    for i in range(0, howmany):
        rec1 = records[random.randint(0, length)]
        rec2 = records[random.randint(0, length)]
        
        accession = '%s_%s' % (rec1.head.split('_')[0],
                               rec2.head.split('_')[0])
        ...
        breakpoint = random.randint(0, minlength)
        
        new_seq = rec1.sequence()[0:breakpoint] +  rec2.sequence()[breakpoint+1:]
        
        new_head = '%s_%s' % (accession, str(breakpoint))
        
        new_rec = dna(new_head, new_seq)
